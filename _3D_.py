...
def look_at(camera_position, target, up):
    ...
    if z_norm > 0:
        ...
        raise ValueError(
            "Zero vector encountered for z_axis during normalization.")
    ...
    if x_norm > 0:
        ...
        raise ValueError(
            "Failed to resolve zero vector for x_axis after adjusting up vector.")
...
def project_to_screen(vertices, camera_position, projection_matrix, view_matrix):
    ...
    ndc_positions = projected_positions[:, :2] / \
        projected_positions[:, 3, np.newaxis]
...
def render_to_terminal_with_culling(boxes, screen_width, screen_height, camera_position, projection_matrix, view_matrix):
    ...
    for box in boxes:
        ...
        screen_x, screen_y = project_to_screen(
            vertices, camera_position, projection_matrix, view_matrix)
        ...
        for i, face in enumerate(faces):
            ...
            if True:
                for i in range(len(face)):
                    ...
                    draw_line(int(screen_x[start]), int(screen_y[start]), int(screen_x[end]), int(
                        screen_y[end]), terminal_screen, screen_width, screen_height)
...
while True:
    ...
    for x in range(-2, 2):
        for y in range(-2, 2):
            for z in range(-2, 2):
                boxes.append(
                    {'center': np.array([x, y_counter+y, z]), 'half_size': 0.5})
    ...
    # ターミナルに立体を表示
    terminal_screen = render_to_terminal_with_culling(
        boxes, screen_width, screen_height, camera_position, projection_matrix, view_matrix)
